...
image::../media/diagram-example.png[width=80%]

⚡️ Extend AsciiDoc with custom extensions:

image::../media/custom-extension-example.png[width=80%]
image::../media/exec-extension-example.png[width=80%]
...
You can configure *ascaid* to load additional extensions by creating a file 
named `ascaid.config.json` in the current working directory. An example can 
be found {repo-root}/examples/asciidoctor-extension[here].

{kroki} is included by default for convenience.

=== GitHub Actions Usage

To publish documents to {confluence} you'll need to create an 
{confluence-api-token} and a root page to publish the
documents under. An example project can be 
found {repo-root}/examples/github-actions-publish-to-confluence[here].
