...
export class ConfluenceClient {
  ...
  async findOrCreatePageByTitleInParent(title, parentPageId) {
    const { data } = await this.#confluenceClient.get(
      `/content/${parentPageId}`
    );

    const confluencePage = data.children.page.results.find(
    ...
    await this.#confluenceClient.post(`/content`, {
      type: "page",
      title,
  ...
  async createConfluencePage(page, confluenceParentPageId) {
    ...
    if (!confluenceParentPageId) {
      const { data } = await this.#confluenceClient.get(
        `/content/${this.#rootPageId}`
      );
      confluencePage = data;
    } else {
      confluencePage = await this.findOrCreatePageByTitleInParent(
        page.title,
